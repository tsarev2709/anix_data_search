...
function App({ session }: { session: Session }) {
  ...
  return <div className="app-shell"><aside className="sidebar"><div className="logo-row"><Brand compact /><div><strong>Anix</strong><small>Contact Search</small></div></div><nav>{navItems.map((item) => <button key={item.id} className={view === item.id ? "active" : ""} onClick={() => setView(item.id)}><i>{item.icon}</i>{item.label}{item.id === "contacts" && pending > 0 && <em>{pending}</em>}</button>)}</nav><div className="sidebar-foot"><span className={activeWorkflow || awaitingWorkflow ? "status-dot working" : data?.status.github ? "status-dot online" : "status-dot"}></span><div><strong>{activeWorkflow || awaitingWorkflow ? "Поиск выполняется" : data?.status.github ? "Система работает" : "Система не настроена"}</strong><small>{latestWorkflow?.current_step || (latest ? `Запуск ${formatDate(latest.finished_at)}` : "Запусков пока нет")}</small></div></div></aside>
